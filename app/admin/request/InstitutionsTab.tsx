...
export const InstitutionsTab = ({ active }: InstitutionsTabProps) => {
  ...
  useEffect(() => {
    const fetchUnverifiedInstitutions = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          toast({ title: "Failed to fetch institutions" });
          return;
        }
  ...
  const handleApprove = async (id: string) => {
    try {
      ...
      const data = await response.json();
      if (!response.ok) {
        toast({ title: data.message || "Failed to approve institution" });
        return;
      }
      ...
      toast({ title: "Internal Server Error --handleApprove" });
    }
  };
  ...
  const handleReject = async (id: string) => {
    try {
      ...
      const data = await response.json();
      if (!response.ok) {
        toast({ title: data.message || "Failed to reject institution" });
        return;
      }
      ...
      toast({ title: "Internal Server Error --handleReject" });
    }
  };

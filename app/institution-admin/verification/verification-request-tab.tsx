...
const VerificationRequestTab: React.FC<VerificationRequestTabProps> = ({
  active,
}) => {
    ...
    const fetchPendingStudents = async () => {
      try {
        ...
        const response = await fetch(
          "/api/institution-admin/verification?status=PENDING"
        );
        const data = await response.json();
        if (!response.ok) {
          setMessage(data.message || "Failed to fetch pending students");
        }
        setPendingStudents(data.students || []);
      } catch (error) {
        console.error("Error fetching pending students:", error);
        setMessage("Internal Server Error -- fetchPendingStudents");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleVerificationUpdate = async (
    ...
    try {
      ...
      const response = await fetch("/api/institution-admin/verification", {
        method: "PUT",
        headers: {
      ...
      const updatedStudents = pendingStudents.filter(
      ...
      setFeedback("");
      setMessage(`The student has been successfully ${status.toLowerCase()}`);
    } catch (error) {
          ...
          <Card key={i}>
            <CardContent className="p-6">
              <div className="flex flex-col gap-6">
                {/* Header with name, email and details */}
              ...
              <div className="flex justify-between space-y-4">
                ...
                <div className="flex flex-col gap-2">
                  <Badge variant="outline" className="text-sm">
                    Enrollment: {student.studentProfile?.enrollmentNo || "N/A"}
                  </Badge>
              ...
              <div className="flex flex-col space-y-4 md:items-end md:justify-end">
                <div className="flex gap-2 w-full md:w-auto">
                  <Button
                    ...
                    onClick={() =>
                      handleVerificationUpdate(student.id, "APPROVED")
                    }
